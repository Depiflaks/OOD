...
Паттерн «Команда» — это поведенческий паттерн проектирования, который инкапсулирует запрос на выполнение действия в виде отдельного объекта. Этот объект, называемый командой, содержит в себе само действие и его параметры, что позволяет отложить выполнение, ставить запросы в очередь, поддерживать отмену/повтор и отделять инициатора запроса от его исполнителя. 
